...
n_threads = 4
tree_depth = 8  # use 16
depth_switch = tree_depth - 4  # use - 6!?
...
# lets do the first 16 rows here
train_ims, train_disps = get_data(n_train_samples, 0, 33, True)  # 33 for good measure
for i in range(0, 16 + 1):
    hd.train_forest(params, train_ims, train_disps, n_disp_bins=n_disp_bins, depth_switch=depth_switch,
                    forest_prefix=str(prefix / f'fr{i}_'), row_from=i, row_to=i + 1, n_threads=n_threads)
for i in range(16, height - 16):
    # load only the stripe that is needed for this current tree
    train_ims, train_disps = get_data(n_train_samples, i - 16, 33, True)  # 33 for good measure (16 in each direction
    hd.train_forest(params, train_ims, train_disps, n_disp_bins=n_disp_bins, depth_switch=depth_switch,
                    forest_prefix=str(prefix / f'fr{i}_'), row_from=16, row_to=17, n_threads=n_threads)

# lets do the last 15 rows here
train_ims, train_disps = get_data(n_train_samples, height - 33, 33, True)  # 40 for good measure
for i in range(height - 16, height):
    row_from = 33 - (i - height)
    row_to = row_from + 1
    hd.train_forest(params, train_ims, train_disps, n_disp_bins=n_disp_bins, depth_switch=depth_switch,
                    forest_prefix=str(prefix / f'fr{i}_'), row_from=row_from, row_to=row_to, n_threads=n_threads)

for i in range(0, height):
    os.rename(str(prefix / f'fr{i}_0.bin'), str(prefix / f'fr{i}.bin'))
...
for tree_depth in [8]:  # , 10, 12, 14, 16]: # probably best results are
    ...
    hd.train_forest(params, train_ims, train_disps, n_disp_bins=n_disp_bins, depth_switch=depth_switch,
                    forest_prefix=str(prefix / 'fr'), row_from=15, row_to=16, n_threads=12)
